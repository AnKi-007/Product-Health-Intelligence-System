...
if releases.empty:
    print("WARNING: No overlapping releases found.")
    print("Ticket and review data cannot be correlated with releases.")
    exit()

# DIFFERENT WINDOWS PER SIGNAL
...
for _, r in releases.iterrows():
    ...
    # Review windows (wider)
    review_before_start = release_date - pd.Timedelta(days=REVIEW_WINDOW_DAYS)
...
# NOTE: If no pre-release reviews exist, we fallback to early post-release reviews
# to establish a sentiment baseline (common in sparse-review scenarios)

    if reviews_before.empty:
        # fallback: first 7 days after release
        reviews_before = reviews[
            (reviews["review_date"] >= release_date) &
            (reviews["review_date"] <= release_date + pd.Timedelta(days=7))
        ]

    reviews_before = reviews_before.tail(1000)
